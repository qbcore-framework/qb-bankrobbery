...
function openLocker(bankId, lockerId) -- Globally Used
    ...
    if bankId == "paleto" then
        QBCore.Functions.TriggerCallback('QBCore:HasItem', function(hasItem)
            if hasItem then
                ...
                IsDrilling = true
                QBCore.Functions.Progressbar("open_locker_drill", Lang:t('progress.breaking_safe'), math.random(18000, 30000), false, true, {
                    ...
                    QBCore.Functions.Notify(Lang:t('success.successful'), "success")
                    IsDrilling = false
                end, function() -- Cancel
                    StopAnimTask(PlayerPedId(), "anim@heists@fleeca_bank@drilling", "drill_straight_idle", 1.0)
                    TriggerServerEvent('qb-bankrobbery:server:setLockerState', bankId, lockerId, 'isBusy', false)
                ...
                QBCore.Functions.Notify(Lang:t('error.too_strong'), "error")
                TriggerServerEvent('qb-bankrobbery:server:setLockerState', bankId, lockerId, 'isBusy', false)
            end
        end, "drill")
    elseif bankId == "pacific" then
        ...
        IsDrilling = true
        QBCore.Functions.Progressbar("open_locker", Lang:t('progress.breaking_safe'), math.random(27000, 37000), false, true, {
            disableMovement = true,
            disableCarMovement = true,
            disableMouse = false,
            disableCombat = true,
                ...
                if closestBank ~= nil then
                    ...
                    if dist < 1.5 then
                        if CurrentCops >= Config.MinimumFleecaPolice then
                            ...
                            QBCore.Functions.Notify(Lang:t('error.minimum_cops', {minCops = Config.MinimumFleecaPolice}), "error")
                        end
                    end
                end
            else
    ...
    if PlayerJob.name == "police" and onDuty then
        ...
        if type == "small" then
            ...
            TriggerEvent('qb-policealerts:client:AddPoliceAlert', {
                timeOut = 10000,
                alertTitle = Lang:t('banks.fleeca') .. Lang:t('info.rob_attempt'),
                coords = {
                    x = coords.x,
                    y = coords.y,
                    z = coords.z,
        ...
        elseif type == "paleto" then
            ...
            TriggerEvent('qb-policealerts:client:AddPoliceAlert', {
                timeOut = 10000,
                alertTitle = Lang:t('banks.blaine') .. Lang:t('info.rob_attempt'),
                coords = {
                    x = coords.x,
                    y = coords.y,
                    z = coords.z,
        ...
        elseif type == "pacific" then
            ...
            TriggerEvent('qb-policealerts:client:AddPoliceAlert', {
                timeOut = 10000,
                alertTitle = Lang:t('banks.pacific') .. Lang:t('info.rob_attempt'),
                coords = {
                    x = coords.x,
                    y = coords.y,
                    z = coords.z,
        ...
        AddTextComponentString(Lang:t('text.blip_name'))
                            ...
                            if not Config.SmallBanks[closestBank]["lockers"][k]["isOpened"] then
                                if lockerDist < 5 then
                                    ...
                                    if lockerDist < 0.5 then
                                        DrawText3Ds(Config.SmallBanks[closestBank]["lockers"][k]["coords"].x, Config.SmallBanks[closestBank]["lockers"][k]["coords"].y, Config.SmallBanks[closestBank]["lockers"][k]["coords"].z + 0.3, '[E] ' .. Lang:t('text.break_safe'))
